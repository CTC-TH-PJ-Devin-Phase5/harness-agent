...
Phase 1 — Grill (`grill-with-docs`) — orchestrator ↔ you, no skip/timeout
           grilling + domain-modeling → notes, docs/CONTEXT.md, docs/adr/
  ▼
...
Phase 4 — per ticket, dependency order:
           4a  execute sub-agent, implement dispatch (loop, max 2 attempts)
               ...
               implement → run `pnpm test:unit` (host Bash)
...
Every ticket's Phase 4 gate is `pnpm test:unit`. `execute` runs that
command on the host via `Bash`, then reports the actual command plus
pass/fail counts in the summary it returns to the orchestrator
(CLAUDE.md § 4a), and logs each attempt as a row in that ticket's own
`## Execution log` table (`.claude/agents/execute.md`). The orchestrator
